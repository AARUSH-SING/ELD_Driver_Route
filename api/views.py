...
class TripPlanAPIView(APIView):
    def post(self, request):
        ...
        def advance_time(minutes):
            nonlocal current_day, current_minute
        ...
        def add_event(status, duration_minutes, label=''):
            nonlocal current_minute, current_day
            ...
            while remaining_minutes > 0:
                ...
                event = {
                    ...
                    'end_minute': current_minute + span,
                }
                clamp_log_event(event, current_day, log_events)
                advance_time(span)
                remaining_minutes -= span
            return event

        def enforce_off_duty(mins=600):
            nonlocal drive_since_break, window_start, window_end
        ...
        for leg in legs:
            ...
            if leg['type'] == 'drive':
                while remain > 0:
                    if current_minute >= window_end or drive_since_break >= 11 * 60:
                    ...
                    if drive_since_break >= 8 * 60:
                        ...
                        drive_since_break = 0
                        continue
                    available = min(remain, (11 * 60 - drive_since_break), (window_end - current_minute))
                    if available <= 0:
                        enforce_off_duty(600)
                        continue
                ...
                while remain > 0:
                    available = min(remain, 24 * 60 - current_minute)
        ...
        for event in log_events:
            ...
            while len(log_days) < day_index:
                log_days.append({'day_label': f'Day {len(log_days) + 1}', 'events': [], 'totals': {'Driving': 0, 'On Duty (Not Driving)': 0, 'Off Duty': 0}})
        ...
        if distance_miles > 1000:
            stops.append({'type': 'Refuel', 'location': 'Fuel stop recommended near midpoint', 'lat': None, 'lon': None})
